...
import * as weathermod from '../fitbit-weather/app'
...
sm.addState(new State({
  ...
  customStart: function() {
    this.event();
    this.poll = setInterval(() => weathermod.fetch, 2000);
    weathermod.fetch(30 * 60 * 1000) // return the cached value if it is less than 30 minutes old 
    .then(weatherdata => {
      console.log(JSON.stringify(weatherdata))
      this.weatherSum.text = weatherdata.description;
      this.location.text = weatherdata.location;
      this.temperature.text = weatherdata.temperatureC;
    })
...
function wake() {
  ...
  timeoutID = setInterval(nextState,2000);
}
